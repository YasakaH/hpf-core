...
24. **Session 002 ran and the Job Status Contract closed the awareness gap (2026-08-05)**: Session 002 (`2026-08-05-0601-microsoft-fara-vs-nodriver-arc`, "Microsoft Fara vs Nodriver: architecture, capabilities, and future of AI browser automation", Blog, deep) ran with the owner's research goal including a Technology Maturity dimension (production readiness, community adoption, maintenance activity, breaking changes, vendor commitment, migration risk) — 6 sources (2 code: `microsoft/fara`, `ultrafunkamsterdam/nodriver`; 2 primary: MS Learn Foundry browser-automation tool, HF Fara1.5-9B model card; 2 HN community payloads), 330 evidence entries, 18 draft findings (6 community signals), released to `exports/sessions`. Friction logged: HN story-title search returned mostly unrelated show-HN submissions for "Fara AI browser" and zero for "Fara1.5" (the connector refused honestly rather than fabricate, exit 10); keyword-density drafts skew toward install/code snippets. The review exposed a missing capability, not a bug: **HPF has no event bus or inbox** — the publishing engine holds state HPF never sees, so HPF cannot present an end-to-end view of work it originated. The fix was the **Job Status Contract** (`hpf-job-status-v0`): a tiny, implementation-independent registry (`tools/hpf-research/jobs.py`, records in `exports/jobs/<job-id>.json` + `index.json`) where each subsystem publishes only its current job state — id, type (research/publishing/marketing/website), owner, status (queued/running/drafting/review/ready/done/blocked/cancelled), started, updated, progress (free-form), research_session, outputs — and HPF consumes those records, never any subsystem's internals. This is the export-contract principle applied in reverse: consumers never know HPF internals, HPF never knows consumer internals. The workbench gained a read-only **Pipeline** view (`#/pipeline`) rendering the chain per session (Research → Publishing → Website → Marketing) with status badges and owner/progress details. Truthful first records registered: Session 001's chain (research/publish/website done), Session 002's research done + publishing queued awaiting review. The registry validates every record (refuses unknown statuses, exits non-zero on invalid records) and the release workflow assembles `exports/jobs` → `website-hpf/jobs`.

25. **Session 002's review confirmed the decisive class claim and HPF crossed into operational-asset territory (2026-08-05)**: the research review adjudicated Session 002 — 1 approve, 4 revised, 13 rejected, 5 added (10 accepted of 18 drafts; Research Yield 56%, raw draft survival 28%). The decisive review question was answered by synthesis: **Fara is a vision-first AI computer-use layer (a model that observes screenshots and emits pixel-grounded actions through a required harness), not a successor to browser automation frameworks; Nodriver is a low-level CDP driver** — the two occupy different abstraction layers of the browser automation stack, so the publish pack was compiled under the reviewer-directed framing **"Microsoft Fara vs Nodriver: Different Layers of the Browser Automation Stack"** (the compiler gained a `--title` publication-title override, a publishing-quality change allowed under the v1 freeze). The end-to-end loop (topic → planning → evidence → classification → draft findings → adjudication → accepted findings → publishing compiler → pack → deployment) has now run twice, and HPF is assessed operational across research, validation, publishing (v1), and measurement. **Editing time is declared the single most important metric** — "how many minutes does an expert need before they're comfortable pressing Publish?" — with a per-render table (article, comparison, LinkedIn, X thread, FAQ, documentation) to be recorded honestly per session. **Rejection-reason categorization was adopted as a second measurement**: across the two sessions' 27 rejections the dominant extraction failure is restatements (13: docs installs, code samples, README blurbs), then off-topic (6), marketing language (4), duplicates (2), nav chrome (1), weak relevance (1) — a roadmap for extraction quality. **Session 002 is declared the classification benchmark**: future engine changes must not collapse the distinction between AI computer-use systems, browser automation frameworks, and browser drivers. **Roadmap B gained a concrete objective**: reduce median manual editing time by improving compilation without changing accepted findings. Engineering effort is directed at the compiler itself (section ordering, transitions, comparison tables, narrative flow, evidence citations, uncertainty handling) — not UI, orchestration, or governance; and the operative question after every session is "what prevented this publish pack from being publishable with minimal edits?"

26. **Editing time became diagnostic and the measurement-driven improvement rule was adopted (2026-08-05)**: the editing-time KPI was made diagnostic — instead of a single opaque total, every edit pass records a **why-editing breakdown** (structure/order, technical accuracy, evidence/citations, narrative flow, grammar/style), tracing each minute to the subsystem responsible: structure and narrative flow point at the Publishing Compiler, technical accuracy at research + adjudication, evidence/citations at the research/evidence chain, grammar/style at compilation. Every improvement is now traceable to a subsystem, keeping Roadmap B actionable — flow-heavy edits mean compiler work, evidence-heavy edits mean upstream research or adjudication work. A further distinction (mandatory edits — incorrect claims, missing citations — vs preference edits — wording, hooks, section order) was identified as the eventual way to answer "was HPF objectively wrong, or did I prefer to write it differently?" and is **explicitly deferred** until a reasonable body of data accumulates. **The programme's governing rule for the operation phase was adopted: never improve HPF because of an idea — improve it because the measurements consistently point to the same weakness.** The instrumented framework is declared coherent and complete: Research Yield (extraction/adjudication), rejection reasons (where extraction fails), editing time (pack usability), edit-category breakdown (which subsystem needs work), Session 001 (regression benchmark), Session 002 (classification benchmark). No further metrics and no further architectural layers will be added; the next 20–30 sessions exist to fill those tables honestly, and the data — not discussion — decides where the next round of engineering effort goes.

27. **The Research Opportunity Engine proposal was considered and parked (2026-08-05)**: a design for scheduled discovery (cron collectors over official, community, and operational sources) → opportunity scoring → a suggested-research queue → owner approval, with cookbook-health staleness as a unique signal source ("which recipes are aging → research trigger"), was reviewed against the measurement-driven rule (entry 26). **No measured weakness yet points to a discovery gap**: Session 002's thin HN signal was connector query quality, not a missed opportunity; no session failed because a development went undiscovered; cookbook staleness is real but unmeasured; and the proposed distinction (discovery frequent, research rare, never autonomous publishing) is sound but untested. The proposal is **parked, not built**, with explicit activation triggers: (1) the owner logs topic-selection friction N times across sessions; (2) a development that materially mattered goes undiscovered (a missed event the owner would have wanted); (3) cookbook-recipe staleness is measured and shown to correlate with reader impact; (4) the post-20–30-session record shows a discovery queue would have changed which sessions ran. The planned cadence (GitHub releases/RSS 6h, HN 12h, Reddit daily, cookbook health weekly, refresh-scan monthly) and the scorecard sketch (domain relevance +40, major vendor +20, GitHub stars +15, HN discussion +15, already researched −50, duplicate −100) are recorded here so the next round of engineering, if the data earns it, starts from the design rather than from scratch.

28. **The decision-classification rule was adopted and the watchlist became executable configuration (2026-08-05)**: a review of the parking decision exposed a process weakness — the proposal had been documented when a part of it was operational configuration that should have been implemented. The rule for all future accepted designs: **classify before responding — governance only → document only; deferred architecture → document only; operational configuration → implement immediately; executable infrastructure → implement immediately; production feature → implement only if activated.** Applied immediately: the **watchlist** (`tools/hpf-research/config/watchlist.yaml`, 3 sections / 26 entries: browser-automation, ai-agents, web-platform) is configuration, not automation — no cron, no discovery, no scoring — and was implemented as such: `tools/hpf-research/watchlist.py` (PyYAML if present, stdlib subset parser otherwise) validates and emits JSON; the research orchestrator loads the watchlist at plan time and logs which watched entries the topic touches (stored in the session plan as `watchlist_matches`); the workbench gained a read-only **Watched Technologies** view (`#/research/watched`) stating the configuration's provenance; the release workflow assembles `data/watchlist.json`. When and if the parked Research Opportunity Engine is activated by measured evidence, this list is its seed rather than a fresh start.

29. **The watchlist was refined into a shared identifier-based service (2026-08-05)**: a follow-up engineering review scored the implementation 8.5–9/10 and identified four refinements, three executed now and one deferred. **Executed**: (a) **identifier-based matching replaced substring matching** — entries now carry explicit `id`/`aliases`/`type` (24 entries, 3 sections) and `match_topic()` matches only on word-bounded alias tokens or exact keyword equality, eliminating false positives like "chrome" matching "chromedriver" (verified by test); (b) **a shared service API** (`watchlist.load()`, `entries()`, `match_topic()` in `watchlist.py`) that every future consumer imports rather than embedding its own copy; (c) **session-artifact persistence** — every new session stores a top-level `watchlist: {"matched": [...], "coverage": <keyword-overlap fraction>}` field, so "show me every session involving Playwright" is answerable without rebuilding anything; `coverage` is explicitly keyword-overlap, not a relevance judgement. **Deferred**: rich seed metadata (priority, sources, RSS/Reddit config) stays out of the configuration until the Research Opportunity Engine is activated — it belongs to the parked engine, not to this file. Matching no longer produces the earlier "Watchlist touches: browserbase, openai" noise on loosely-overlapping keywords; the plan-stage log and session field are now precise.

30. **The watchlist moved to a versioned, validated schema with vendor-independent ids (2026-08-05)**: a second engineering review rated the implementation 9.7–9.8/10 and offered five long-term-maintainability refinements, framed as refinements rather than design flaws. Classified under entry 28, four were **operational configuration / executable infrastructure → implemented immediately** (migration cost was zero: no released session yet carries a watchlist id), one was **a measurement note → documented only**. **Implemented**: (a) **stable vendor-independent canonical ids** — `tech.<domain>.<name>` (e.g. `tech.browser.fara`), with a separate display `name` field, so a vendor transfer (microsoft/fara → azure/fara) never rewrites history; (b) **alias normalization at load** — every alias, name, topic, and keyword is normalized (lowercase, non-alphanumeric runs → single space) so `playwright-python`/`playwright_python`/`Playwright Python` all become `playwright python`, and matching happens on one normalized surface; (c) **schema versioning** — `schema: hpf-watchlist-v1` at the top of the YAML, emitted in `to_json()`, enforced at load; (d) **startup validation that fails fast** — duplicate ids, duplicate normalized aliases, missing aliases, missing type, and empty sections all raise `ValueError` on load (and `--check` reports them), so configuration drift is impossible silently. Matching additionally uses each entry's normalized display name as a match surface (e.g. topic "Selenium" matches `tech.browser.selenium` whose alias is the two-token "selenium webdriver"). The test suite was extended to 15 watchlist checks (8 matching + schema + 6 validation rejections), all green. **Documented only**: the reviewer's caution that `coverage` depends on keyword generation — it is a debugging metric, not something to compare across sessions unless keyword generation is frozen; this is recorded in the `watchlist.py` docstring and here so no future analysis treats cross-session coverage deltas as signal. The workbench watchlist view now renders display names with canonical ids as chip titles; session rows resolve matched ids to display names at render time.

31. **The watchlist was declared v1-mature and the engineering focus shifted back to research operations (2026-08-05)**: a third review rated the implementation production-quality (decision making 10/10, architecture 9.6/10, long-term maintainability 9.7/10, production readiness 9/10) and explicitly directed that **engineering time on the watchlist stop** — it is "good enough" unless a real research session exposes a concrete defect. Two actions resulted. **Implemented (executable infrastructure, migration-free)**: the session field `coverage` was renamed to `keyword_overlap` — the review's point that the old name invited cross-session comparison the documentation explicitly forbids; the name now says literally what the metric measures. Renamed across `match_topic()`, the session plan field, the plan-stage log, the workbench session row, and the test suite; the release-pipeline JSON shape is unaffected. **Documented only (deferred architecture, not for v1)**: five future refinements, with their activation conditions, are recorded here rather than built: (a) **hierarchical matching** (framework → plugin → extension → SDK → protocol) so parent technologies do not always match when a child appears — not urgent; (b) **explicit entry lifecycle** (active/deprecated/archived/experimental/parked/removed) to keep long-term history clean; (c) **entry relationships** (`depends_on`, `implements`, `vendor`, `related_to`, `category`) for future search, graphs, and impact analysis; (d) **immutable ids separated from canonical paths** (`id: wl-00172` + `canonical_path: tech.browser.playwright`) so category renames never change identity; (e) **weighted aliases** (primary/secondary/deprecated/nickname/abbreviation) — e.g. "MCP" is weaker than "Model Context Protocol". All five are parked with this entry; none are operational configuration or infrastructure today. Also recorded: the review's explicit **do-not-build list** — priorities, RSS URLs, discovery scheduling, scoring, tags, dashboards, watch statistics, heatmaps — which belong to the parked Research Opportunity Engine (entry 27), not to the watchlist. The programme's return on engineering now sits in the measured domain: running real sessions (20–30 for the measurement framework), collecting editing-time data for Roadmap B, and improving extraction (Research Yield ~56%) and adjudication (restatements, duplicates, marketing) — those measurements, not the watchlist, decide the Opportunity Engine's activation.

32. **Discovery activation: the watchlist gained `sources` and the minimal manual `discover.py` was built (2026-08-05)**: a fourth review redirected the programme — "stop optimizing configuration, start optimizing intelligence" — and ruled that the parked Research Opportunity Engine (entry 27) is **earning activation**. The activation evidence is documented, not asserted: Session 002's topic ("Microsoft Fara vs Nodriver…") and all four primary URLs (github.com/microsoft/fara, ultrafunkamsterdam/nodriver, MS Learn Foundry browser-automation, HF Fara1.5-9B model card) were **owner-supplied**; Microsoft Fara (released 2026-07-22, materially relevant to the programme's core domain) was unknown to HPF until the owner brought it. That satisfies activation trigger (2) of entry 27: *a development that materially mattered went undiscovered*. Classified under entry 28: **operational configuration → implemented immediately; minimal executable precursor → implemented as such; the full engine stays parked**. **Implemented**: (a) **`sources` on watchlist entries** — WHERE TO LOOK per technology (`github_releases`, `docs`, `blog`, `npm`, `pypi`, `hf`, `reddit`, `hackernews`; fixed vocabulary enforced at load, non-empty string lists; entries without sources allowed). This corrects the deferral recorded in entries 29–30 — sources were reclassified as operational configuration on activation evidence, not engine automation: inert data, nothing runs against it autonomously, and the workbench watchlist view renders them; (b) **`tools/hpf-research/discover.py`** — manual, on-demand, **no cron, no scoring, no queue, no autonomous anything**: `--static` prints the suggestion seed list from the watchlist (sections + entries + sources, exactly the review's "Suggested Topics" sketch without invented priority), `--releases` fetches each `github_releases` repo's latest GitHub release via the API (mechanical, no LLM, graceful 404/rate-limit messages) and prints "today's opportunities" — a live run surfaced Stagehand browse@0.9.6 (2026-08-03), Puppeteer browsers v3.1.0 (2026-08-04), and Microsoft Foundry dotnet-1.17.0 (2026-08-04) unprompted; `--out` writes suggestions JSON to a path the owner chooses. Owner chooses, HPF researches. **Recorded, not built (measurement discipline + feature freeze)**: the review's further priorities — connector expansion (Google AI blog, Chrome Developers, Microsoft DevBlogs, npm/PyPI trends) is allowed only when a session fails without them (feature-freeze clause); extraction-quality engineering (Research Yield ~56%) and drafting/compiler work (Roadmap B) are the standing quality targets, to be driven by the 20–30-session measurement record, not by design ideas. Also recorded as the programme's allocation rule from this review: **every engineering change must answer "does this help HPF discover better research, produce better findings, or reduce publishing edits?" — if no, it waits**, no matter how elegant.

33. **Architecture and governance round: Research Event model adopted; chronicle/change-log separation (2026-08-05)**: a fifth review rated the direction correction 9.8/10 (architecture), 8.8/10 (product thinking) and issued the next roadmap. **Decisions recorded here; implementation detail in the new `ENGINEERING_CHANGE_LOG.md` (separation policy below).** (a) **Research Event model adopted** — every discovery connector produces the same unified object (`research-event-v0`: id, title, source, date, event_type, technology, link, summary, discovered_at), so discover.py "discovers events", not releases; GitHub, docs, RSS, HN, Reddit all become producers of the same object. Event ids are deterministic across runs so the queue can remember them. (b) **Connector abstraction adopted** — a registry of connectors returning ResearchEvent[]; the GitHub Releases connector is the first and the old `--releases` mode was renamed `--events` accordingly. (c) **Opportunity queue adopted** — statuses new/ignored/researched/duplicate/parked/expired, persisted in the git-ignored working area; discover.py never re-shows a dispositioned event as new. (d) **Watchlist schema frozen again** — no new fields unless a real research session exposes a concrete need; the reviewer's connector-registry CONFIG separation (a file describing each connector's data sources apart from the watchlist) is deferred with this trigger: build it when a second connector needs configuration beyond the watchlist `sources` vocabulary. (e) **Ranking/prioritization (Phase 5) explicitly deferred** — scoring, priority tiers, recommendations, and automatic suggestions come only after real usage data; today's events are unranked by design. (f) **Chronicle/change-log separation policy** — the chronicle records major architectural decisions and governance changes only; implementation details (schema updates, validation additions, renames, tests) go to `ENGINEERING_CHANGE_LOG.md`, which backfills entries 29–32's engineering detail. Historical entries are not rewritten; the policy applies forward. The reviewer's framing is recorded as the programme's operating principle for this area: the watchlist describes WHAT; the connector registry describes where connectors obtain data; the watchlist does not accumulate connector configuration.

34. **Model validation: a second connector (blog/RSS) was built; infrastructure declared closed pending real usage (2026-08-05)**: a sixth review scored the round 9.9/9.7/9.4/9.5 and directed the programme to **stop adding infrastructure and exercise the abstractions**: build exactly one more connector to test whether the ResearchEvent model generalizes, then run real research sessions and let empirical friction — not anticipated complexity — drive the next iteration. **Built (the validation connector)**: a **blog/RSS connector** in the discovery registry — it resolves each watchlist `blog:` source to a feed (value may be a blog root, probed across common feed paths `rss.xml/feed.xml/atom.xml/index.xml/rss/feed/feeds/posts/default`, or a direct feed URL), parses RSS 2.0 and Atom with the stdlib only, and emits ResearchEvent[] with `event_type: blog`, skipping items without parseable title or date honestly. No schema change (source values are free-form strings; a direct feed URL also works) and no new dependencies or auth. The model validated: a live run produced real, relevant blog events alongside release events — Cloudflare OS and The Agent Access Model (2026-08-05), Chrome's agent-ready toolkit (2026-06-22), WebKit Safari 26.6 (2026-07-27) — with unresolvable feeds (playwright.dev/blog, anthropic.com/research, blog.mozilla.org) reported honestly as "no RSS feed found" for the owner to correct with direct feed URLs later. **Documented only (deferred architecture, with triggers — not built, per the same measurement discipline)**: (a) **Observation model** — ResearchEvent evolving toward an Observation (evidence/confidence/artifacts) so a blog post + release + HN thread about the same change deduplicate ABOVE the connector layer; trigger: multi-source events about one change recurring in real sessions; (b) **Disposition on events** — queue status growing into notes/decision/owner/reason/session/date; trigger: queue status alone proving insufficient; (c) **connector capabilities metadata** (rate limits, cost, supports_incremental/search/history); trigger: a registry consumer needing them; (d) **semantic event identity** — identity depending on canonical event semantics rather than transport URLs; trigger: observed URL instability. The connector-config split trigger from entry 33 stands. **Standing discipline**: the next engineering round is not planned — it is earned by running Session 003 and onward through this pipeline, recording where the model breaks, and by the 20–30-session measurement record (editing-time tables for Roadmap B remain the first unfilled data point).

35. **Discovery v1 declared genuinely frozen; the Research Intelligence distinction and the post-30-session review instrument adopted (2026-08-05)**: a seventh review scored the final discovery round 9.9/10 (10/10 across architecture, measurements, implementation, simplicity; 9.8 maintainability; 9.5 operational readiness) and its instruction to the coder is recorded verbatim as the programme's operating directive: **stop discovery engineering; discovery is frozen; run 20–30 real sessions; record every measurement honestly; do not add any subsystem; after 30 sessions produce a review answering which connectors generate the best research, which sources are ignored most, which topics generate the best publish packs, which sessions require the least editing, which rejection reasons dominate, and which technologies repeatedly appear together — and only then decide whether a Research Intelligence layer is warranted.** The review drew a strategic boundary distinction that is recorded as governance: HPF is no longer missing **discovery** (events are produced and frozen); it is missing **intelligence** — the capability to answer "what should I research tomorrow?" without manual decision. The two are separated: **System A (Discovery, finished)**: GitHub/RSS/Blogs/Manual/HN → Events — frozen, never re-opened absent measured failure; **System B (Research Intelligence, not built)**: Events → Correlation → Priority → Research Recommendation — explicitly deferred, gated on the post-30-session review. The reviewer's synthesis example is recorded as the target's shape: when Chrome releases an AI toolkit, Microsoft releases Fara, Browserbase publishes agent APIs, Anthropic updates computer use, and Playwright adds agent support, HPF should notice the connection itself — that is synthesis, not discovery. **Documented only (deferred, each with its gate)**: (a) **event themes** — events carry technology/source/date but no theme (AI Agents, Browser Automation, Vision Models, CDP, Anti Detection, Tool Calling, MCP, Benchmarks, Browser Security), so HPF cannot yet say "five vendors moved toward agentic browser automation this week"; (b) **event → finding → corpus-object invalidation** — which corpus knowledge an event made obsolete (cookbook maintenance value); (c) **UI direction shift** — the backend is mature and the next UI effort (timeline, opportunity inbox, evidence explorer, relationship graph, research queue, session comparisons) is parked behind operational evidence rather than built on an engineering-looking workbench; (d) **the analytics feedback loop (v2)** — research → article → Search Console → analytics → reader behaviour → research priorities, making HPF a true learning system; explicitly not now. The governance decisions of the final round, in force from this entry: the connector table's metric is **F/Event** (findings per event, a ratio that may exceed 100% by design), **S/Event** (sessions per event, answering "did this connector inspire research?" separately from "did it produce findings?"), and **Ignored** (queue-dispositioned, distinguishing discovered from actually investigated); multi-connector session attribution is directional and non-additive by contract. The pipeline conversion report (connector → published pack, drop-off at every stage) is **postponed** until operational data makes it meaningful. From this entry, engineering effort belongs to research quality, extraction/adjudication, and publishing (Roadmap B) — driven only by measured friction from real sessions; the 20–30-session record, not design discussion, decides the Research Intelligence question.
36. **Review round 10: extraction subsystem declared NOT closed; measurement-hardening implemented, architectural redesign staged with triggers (2026-08-05)**: the lead-architect review of the extraction round (round 9) scored it "excellent tactical engineering" (root-cause 10/10, tactical implementation 9.5/10, test discipline 9.5/10, architectural maturity 7.5/10, long-term maintainability 7/10, extensibility 7.5/10) and ruled explicitly: **accept the work, but do NOT close the extraction subsystem.** The review's central claim is recorded as governance: the pipeline is still **text-centric (paragraphs) rather than knowledge-centric (claims/knowledge units)** — paragraph filtering, prose heuristics (length >= 40, contains "."), and character-based coverage all assume document style. The review's recommendations were classified under entry 28 and split into two responses. **Implemented immediately (measurement infrastructure — the failure taxonomy the review demanded)**: per-source extraction metrics now recorded in every session (truncated, duplicates cross-source, boilerplate_ratio, avg_para_chars, largest_para_chars, paragraphs pre-filter count) via a shared testable extract_source() helper with a session-level seen set; a session-wide **extraction health line** (boilerplate_ratio, duplicate_ratio, truncated count, avg/largest paragraph) printed and logged as a longitudinal regression signal; the **extraction-underperformance flag normalized** — fires only when adds > 3 AND adds/accepted >= 0.35 (the review's point that raw counts conflate small and large sessions) and now carries accepted_total, adds_accepted_ratio, both thresholds, and a reviewer-supplied causes taxonomy (missing_claims, duplicate_claims, truncated_claims, low_confidence_claims, boilerplate_leakage, coverage_loss, source_dominance, missing_urls, keyword_generation); block-tag list hardened with the structural elements the review named (details, summary, figure, figcaption, aside, main, dl, dt, dd, ol, ul, nav). Test discipline: golden-corpus-style test14 (12 offline checks) added; test10-13 rerun green. **Documented only — staged with triggers, NOT built (the measurement discipline: one extraction-round session of data does not justify a redesign)**: (1) **knowledge units as the pipeline primitive** (paragraph -> list/table-row/definition/code-explanation/quote; coverage -> accepted-units/candidate-units) — HIGH, the next extraction redesign; trigger: golden corpus or sessions 004-010 showing list/table/definition content loss via the new unit metrics; (2) **adaptive budgets** (budget = f(claim density x source quality x novelty x importance)) — trigger: claim-detection stage exists (it needs claims to count density); (3) **boilerplate scoring** (link/text/sentence density, unique-word ratio, navigation probability, DOM position, heading proximity -> 0-1 score) — trigger: pattern count passes ~50 or golden corpus shows footer leakage; Session 004's site-tagline boilerplate class recorded as a known pattern gap, deliberately NOT added this round; (4) **semantic/concept-level overlap** (concept ids or embeddings over keyword equality) — trigger: a real session where normalized lexical overlap misleads; (5) **confidence propagation on evidence** (confidence/novelty/source authority -> non-binary adjudication) — trigger: knowledge units; (6) **evidence deduplication before adjudication** — trigger: the new cross-source duplicate_ratio exceeding ~0.15 in real sessions (S004 first data point: 0.00); (7) **plugin extractor architecture** (Extractor strategy interface: Html/Markdown/Pdf/Repository/ApiDoc) — trigger: a second content type beyond HTML/markdown import showing real demand; (8) **claim coverage** — derivative of knowledge units, staged with (1). Operating context unchanged: discovery remains frozen (entry 35); the extraction subsystem now carries its own review gate — the post-30-session review must weigh whether extraction or discovery caused each session's measured losses.
37. **Review round 11: metric-honesty renames, weighted telemetry, and the real-world golden corpus; knowledge-unit trigger operationalized (2026-08-05)**: the implementation review of round 10 (fidelity 9.5/10, engineering 9/10, architecture 7.5/10, maintainability 7/10, measurement 8.5/10) accepted the round, kept the extraction subsystem open, approved proceeding to sessions, and set the highest-priority future work: build a corpus of real-world regression fixtures and collect enough telemetry to justify or falsify the move from paragraph-centric to knowledge-centric extraction. Classified under entry 28. **Implemented immediately**: (a) **coverage renamed text_retention** — kept_chars/total_chars measures text retention, not information retention; telemetry, never a success metric (the released S004 artifact is immutable and untouched; its usage-log record carries the rename note); (b) **duplicates renamed exact_duplicates** — the implementation detects exact normalized-prefix reuse, not semantic duplicates; semantic dedup (MinHash/SimHash/embeddings/claim fingerprints) stays staged with its trigger; (c) **boilerplate_ratio renamed chrome_ratio** — vocabulary consistency; parser-level loss separately visible via text_retention (the review's denominator-bias point); (d) **weighted health averages** — avg_para now computed from total kept chars / total kept paragraphs, so a 3-paragraph source no longer weighs as much as a 300-paragraph one; (e) **budget telemetry** — budget_state (saturated/full/partial) + budget_overflow_prevented per source, saturated count in the health line: the data that will justify or falsify the budget constants (40/10/20/25/15) before adaptive budgets; (f) **structured-content drop instrumentation** — split_paragraphs returns (paras, dropped, structured_dropped) for dropped chunks that looked like list items / table rows / definitions, which OPERATIONALIZES the knowledge-unit activation criterion: unit loss is now measurable (chronicle 36 item 1 no longer risks being forgotten); (g) fetch_ms per source; (h) light SRP decomposition of extract_source into split -> budget -> paragraph_metrics with norm_excerpt_key extracted. **Real-world golden corpus built (the review's #1 priority)**: six frozen real pages committed at tools/hpf-research/tests/fixtures (Chrome blog, cloudflare-os README, MDN element reference, Wikipedia article, RFC 6455, arXiv abstract) with a fixtures.json manifest of expected extraction numbers; test15 (19 checks) exact-matches fresh extraction against the frozen pages; regeneration protocol recorded (manifest regenerates only when the pipeline intentionally changes, committing both together). **First real-corpus measurements**: RFC 6455 shows 474 paragraphs / 547 chrome / 89 structured drops / retention 0.111 — structured loss at scale on RFC-style documents is real and measurable; Wikipedia and RFC saturate the primary-40 budget; README saturates code-10 at retention 0.067; the Chrome blog keeps all 32 paragraphs at 0.525. **Recorded only (staged)**: duplicate directionality logging (per-source exact_duplicates already exposes the asymmetry), source entropy, claim density, rejection-reason counts, memory consumption (knowledge-unit-dependent); the BLOCK tuple remains the documented seam for tag evolution, the strategic fix being the semantic classifier already staged with knowledge units. Operating context: discovery frozen (entry 35); extraction gated as before; next sessions 005+ run against the hardened metrics.
